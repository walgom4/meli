...
const getItems = async (query, limit = 4, offset = 0) => {
  return await axios.get(`${BASE_URL}/sites/MLA/search?q=${query}&limit=${limit}&offset=${offset}`)
}
...
app.prepare().then(() => {
  ...
  server.get('/api/items', function (req, res) {
    ...
    if (req.query.q) {
      getItems(req.query.q, req.query.limit ? req.query.limit : null, req.query.offset ? req.query.offset : null).then(response => {
        try {
          res.json(utils.formatItems(response.data));
        } catch (error) {
          console.log(error);
  ...
  server.get('/api/items/:id', function (req, res) {
    ...
    if (req.params.id) {
      getItemDetail(req.params.id).then((response) => {
        try {
